...
The Dota match audit is the first explicit provider-backed replay loop. A player supplies one public Match ID and starts a read-only request deliberately, then selects a hero without exposing account identifiers, player names or chat. One sanitized same-origin relay retrieves and reduces the public OpenDota response; it has no database and returns `no-store`. Parsed matches compare the selected player's gold phases with one unambiguous same-position opponent and the full-team economy, then identify one material replay window through published thresholds. Completed-item timings remain supporting evidence rather than the diagnosis. Unparsed matches stay at an honest final snapshot and never receive an invented timestamp or cause. A shared result carries only the public Match ID, player slot and chosen role, and never starts a request automatically unless the player explicitly submits it from a Money Meta match form.
