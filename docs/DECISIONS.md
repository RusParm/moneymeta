...
## 2026-08-11: static-first architecture

Use Astro and TypeScript. Avoid backend/auth complexity until local profiles and recurring utility are validated.

## 2026-08-11: data trust architecture

Every material metric carries a verification status and check date. Stale weekly data must never be presented as current.

## 2026-08-11: routes

Russian lives at the root; English lives under `/en/`. Existing `.html` calculator traffic receives an explicit permanent redirect to the clean route.
...
## 2026-08-12: Dota scenarios represent distinct decision classes

Midas and buyback are deep calculable mechanics, but they do not define the whole match economy. The hub must also cover farming capacity, acceleration versus fighting power, initiation access, support utility, objective conversion and replay review. Prepared scenarios are useful only when they expose materially different decisions.

## 2026-08-12: WoW is a market operating system

The WoW hub is organized around the full player-market loop: sourcing, crafting, listing, sell-through, orders, liquid gold and reinvestment. Crafting margin and farm liquidity remain foundational, while commission floors, conditional route rankings, inventory decisions and current official changes make the hub useful across player types.
...
## 2026-08-19: recurring utility must remain game-native

A shared calculation engine may normalize target, liquid starting state, protected reserve, sustainable pace and horizon, but the player-facing model must speak the economy of its game. GTA uses weekly net cash, Dota uses current minute and GPM, WoW uses liquid gold and effective GPH, Total War uses turns and campaign treasury, and Crusader Kings III uses months and succession buffer. Shared structure is allowed; generic advice is not.
