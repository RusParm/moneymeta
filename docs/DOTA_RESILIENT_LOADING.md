...
- One read at a time. An explicit Cancel loading action aborts the browser wait immediately; late results and errors cannot mutate a newer request. Leaving the page cancels silently. This is not a promise that an upstream server stopped processing its read.
