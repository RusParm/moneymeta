...
describe("v1.9 hub portals", () => {
  ...
  it("keeps Dota hero marks inside the compact section rail icon column", () => {
    ...
    expect(portalSource).toMatch(/@media \(max-width:\s*1100px\)[\s\S]*?\.portal-section-rail \.hub-game-mark \.dota-media\s*\{[^}]*width:\s*40px;[^}]*height:\s*38px;[^}]*flex-basis:\s*40px;/u);
  });
});
